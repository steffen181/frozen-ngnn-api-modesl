...
def _resolve_artifact(artifact_path: str | Path | None, cache_dir: str | Path | None) -> Path:
    if artifact_path is not None:
        return Path(artifact_path)
    directory = Path(cache_dir) if cache_dir is not None else Path.home() / ".cache" / "ngnn"
    ...
    try:
        ...
        raise NgnnGeneralEncoderError("Could not download public NGNN artifact") from None
    if hashlib.sha256(data).hexdigest() != ARTIFACT_SHA256:
...
def load_ngnn_model(
    ...
    **kwargs: Any,
    ...
    if torch.__version__.split("+")[0] != "2.11.0":
        raise RuntimeError("This NGNN revision requires torch==2.11.0 for top-k tie semantics")
